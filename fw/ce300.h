#pragma once

#include "fw/aux_common.h"
#include "fw/millisecond_timer.h"
#include "fw/stm32g4_dma_uart.h"

namespace moteus {

class Ce300 {
 public:
  Ce300(const aux::UartEncoder::Config& config,
        Stm32G4DmaUart* uart,
        MillisecondTimer* timer)
      : config_(config),
        uart_(uart),
        timer_(timer) {}

  void ISR_Update(aux::UartEncoder::Status* status) MOTEUS_CCM_ATTRIBUTE {
    const uint32_t now_us = timer_->read_us();
    const uint32_t delta_us = (now_us - last_query_start_us_);

    if (query_outstanding_) {
      if (delta_us > static_cast<uint32_t>(2 * config_.poll_rate_us)) {
        // Timeout
        uart_->finish_dma_read();
        query_outstanding_ = false;
      } else {
        ProcessQuery(status);
      }
    }

    if (query_outstanding_) { return; }

    if (delta_us < static_cast<uint32_t>(config_.poll_rate_us)) {
      return;
    }

    last_query_start_us_ = now_us;
    query_outstanding_ = true;

    // Build and send request packet
    uint8_t req[6];
    req[0] = 0xEE; // CE300_HEAD_REQ
    req[1] = seq_++;
    req[2] = 0x0B; // CE300_CMD_READ_ABS
    req[3] = 0x00;
    uint16_t crc = CalcCRC16_Modbus(req, 4);
    req[4] = crc & 0xFF;
    req[5] = (crc >> 8) & 0xFF;

    for (int i = 0; i < 6; i++) {
      uart_->write_char(req[i]);
    }
    
    StartRead();
  }

  void ProcessQuery(aux::UartEncoder::Status* status) MOTEUS_CCM_ATTRIBUTE {
    // We expect 9 bytes for the response
    if (uart_->read_bytes_remaining() > kResyncBytes) { return; }

    if (uart_->read_bytes_remaining() == 0) {
      uart_->finish_dma_read();
      query_outstanding_ = false;
      return;
    }

    if (buffer_[0] != 0xEC || buffer_[2] != 0x0B || buffer_[3] != 0x03) {
      return; // Invalid header, wait for timeout
    }

    uart_->finish_dma_read();
    query_outstanding_ = false;

    uint16_t crc_calc = CalcCRC16_Modbus(buffer_, 7);
    uint16_t crc_rx = buffer_[7] | (buffer_[8] << 8);

    if (crc_calc != crc_rx) {
      return; // CRC Error
    }

    if (buffer_[1] != static_cast<uint8_t>(seq_ - 1)) {
      return; // Seq mismatch
    }

    uint8_t fault = buffer_[4];
    uint16_t raw = buffer_[5] | (buffer_[6] << 8);

    if (fault != 0) {
      return; // Fault indicated
    }

    // Since CPR is 65536, we shift it to match moteus 32-bit internal format if needed,
    // actually `status->value` is typically a 32-bit value.
    // For Aksim2 (18 bit + status) it was assigned raw value and moteus core handles cpr.
    status->value = raw;
    status->nonce++;
    status->active = true;
  }

 private:
  void StartRead() MOTEUS_CCM_ATTRIBUTE {
    uart_->start_dma_read(
        mjlib::base::string_span(reinterpret_cast<char*>(&buffer_[0]),
                                 sizeof(buffer_)));
  }

  static uint16_t CalcCRC16_Modbus(const uint8_t* Frame, uint16_t Len) MOTEUS_CCM_ATTRIBUTE {
    static const uint8_t CRCHi[] = {
        
    };

    static const uint8_t CRCLo[] = {
        
    };

    uint8_t ucCRCHi = 0xFF;
    uint8_t ucCRCLo = 0xFF;
    int iIndex;

    while (Len--) {
      iIndex = ucCRCLo ^ *(Frame++);
      ucCRCLo = (uint8_t)(ucCRCHi ^ CRCHi[iIndex]);
      ucCRCHi = CRCLo[iIndex];
    }
    return (uint16_t)(ucCRCHi << 8 | ucCRCLo);
  }

  const aux::UartEncoder::Config config_;
  Stm32G4DmaUart* const uart_;
  MillisecondTimer* const timer_;

  bool query_outstanding_ = false;
  uint32_t last_query_start_us_ = 0;
  uint8_t seq_ = 0;

  static constexpr int kResyncBytes = 3;
  // CE300 response is 9 bytes
  uint8_t buffer_[9 + kResyncBytes] = {};
};

}
